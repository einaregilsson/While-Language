...
namespace Demo
{
    public class IronyLanguageService : Microsoft.VisualStudio.Package.LanguageService
    {
        ...
        {
            grammar = new Grammar();
            compiler = new LanguageCompiler(Configuration.Grammar);
            context = new CompilerContext(compiler);
            context.Options = CompilerOptions.MatchBraces | CompilerOptions.CollectTokens;
        }
        ...
        public override Microsoft.VisualStudio.Package.AuthoringScope ParseSource(ParseRequest req)
        {
            ...
            switch (req.Reason)
            {
                case ParseReason.Check:
                    ...
                    SourceFile sourceFile = new SourceFile(req.Text, req.FileName);
                    AstNode node = compiler.Parse(context, sourceFile);
                    source.ParseResult = node;

                    // Used for brace matching.
                    TokenList braces = EditorUtilities.GetOpeningBraces(context.Tokens);
                    ...
                    {
                        foreach (SyntaxError error in context.Errors)
                        {
